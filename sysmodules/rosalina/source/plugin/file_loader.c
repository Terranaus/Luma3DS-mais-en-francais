...
bool     TryToLoadPlugin(Handle process, bool isHomebrew)
{
    ...
    if (!res && R_FAILED(res = Check_3gx_Magic(&plugin)))
    {
        const char * errors[] = 
        {
            ...
            "Fichier de plugin obsolete\nRechercher un plugin plus recent.",
            "Chargeur de plugin obsolete\nVerifiez les mises a jour de Luma3DS."   
        };
    ...
    if (!res && fileHeader.infos.compatibility == PLG_COMPAT_EMULATOR) {
        ctx->error.message = "Le plugin est uniquement compatible avec les emulateurs";
        res = -1;
    }
    ...
    if (!res && R_FAILED((res = MemoryBlock__SetSize(memRegionSizes[fileHeader.infos.memoryRegionSize])))) {
        ctx->error.message = "Impossible de definir la taille du bloc de memoire.";
    }
    
    // Ensure memory block is mounted
    if (!res && R_FAILED((res = MemoryBlock__IsReady())))
        ctx->error.message = "Echec de l'allocation de memoire.";
    ...
    if (!res && R_FAILED(res = Read_3gx_LoadSegments(&plugin, header, ctx->memblock.memblock + sizeof(PluginHeader)))) {
        if (res == MAKERESULT(RL_PERMANENT, RS_INVALIDARG, RM_LDR, RD_NO_DATA)) ctx->error.message = "Ce plugin necessite une fonction de chargement.";
