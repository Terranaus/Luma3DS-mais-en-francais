...
static int ERRF_FormatError(char *out, const ERRF_FatalErrInfo *info, bool isLogFile)
{
    ...
    switch (info->type)
    {
        ...
        case ERRF_ERRTYPE_LOG_ONLY:
            ...
            out += sprintf(out, "Code erreur :       0x%08lx\n", info->resCode);
            break;
        ...
        case ERRF_ERRTYPE_FAILURE:
            ...
            break;
        default:
            out += sprintf(out, "Donnees d'erreur fatale non valides.\n");
    }
...
static void ERRF_DisplayError(ERRF_FatalErrInfo *info)
{
    ...
    posY = Draw_DrawString(10, posY + SPACING_Y, COLOR_WHITE, "Appuyez sur un bouton pour redemarrer.");
