...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(ConvertVAToPA)
{
    ...
    else
    {
        ...
        if(R_FAILED(r))
        {
            n = sprintf(outbuf, "Processus invalide (wtf ?)\n");
            goto end;
        }
        ...
        if (R_FAILED(r))
        {
            n = sprintf(outbuf, "Une erreur s'est produite : %08lX\n", r);
            goto end;
        }
    }
...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(SyncRequestInfo)
{
    ...
    if(ctx->selectedThreadId == 0)
    {
        n = sprintf(outbuf, "Impossible d'executer cette commande sans thread selectionne.\n");
        goto end;
    }
    ...
    {
        n = sprintf(outbuf, "Thread non valide ou en cours d'execution.\n");
        goto end;
    }
    ...
    {
        n = sprintf(outbuf, "Le thread selectionne n'execute actuellement pas de demande de synchronisation (svc 0x32).\n");
        goto end;
    }
    ...
    if(R_FAILED(r))
    {
        n = sprintf(outbuf, "Identifiant invalide.\n");
        goto end;
    }
...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(ListAllHandles)
{
    ...
    {
        n = sprintf(outbuf, "handles Trouve %ld.\n", count);
...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(GetMemRegions)
{
    ...
    {
        posInBuffer = sprintf(outbuf, "Processus invalide (wtf ?)\n");
...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(ToggleExternalMemoryAccess)
{
    ...
    n = sprintf(outbuf, "Acces a la memoire externe %s avec succes.\n", ctx->enableExternalMemoryAccess ? "active" : "desactive");
...
GDB_DECLARE_REMOTE_COMMAND_HANDLER(GetThreadPriority)
{
    ...
    n = sprintf(outbuf, "Thread (%ld) priorite : 0x%02lX\n", ctx->selectedThreadId,
...
GDB_DECLARE_QUERY_HANDLER(Rcmd)
{
    ...
    char *endpos;
    const char *errstr = "Commande non reconnue.\n";
